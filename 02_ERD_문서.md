...
> **연관 문서:** `03_아키텍처_정의서.md`, `04_API_명세서.md`, `07_요구사항_정의서.md`, `18_코드_품질_게이트.md`, `23_도메인_용어사전.md`
...
| 테이블 | 주요 컬럼 | 설명 |
| --- | --- | --- |
| `users` | `id`, `display_name`, `email`, `role`, `status`, `created_at` | 일반 사용자와 관리자 계정 |
| `user_auth_identities` | `id`, `user_id`, `provider`, `provider_user_id`, `email` | Google OAuth 연결 정보 |
| `refresh_tokens` | `id`, `user_id`, `jti`, `expires_at`, `revoked_at` | Refresh Token 블랙리스트와 만료 관리 |
...
| `today_qt_cache_entries` | `id`, `qt_date`, `cache_key`, `expires_at`, `payload_hash`, `refreshed_at` | Today QT 캐시 관리용 메타데이터 |
...
| 테이블 | 주요 컬럼 | 설명 |
| --- | --- | --- |
| `journals` | `id`, `user_id`, `qt_range_id`, `feeling`, `memory_verse`, `application`, `prayer`, `visibility`, `created_at`, `updated_at` | 오늘 QT 기준 묵상 노트 |
...
| `anonymous_shares` | `id`, `journal_id`, `user_id`, `status`, `shared_at` | 선택적 익명 공유 |
| `share_comments` | `id`, `share_id`, `user_id`, `body`, `created_at` | 공유 글 댓글 |
| `share_likes` | `id`, `share_id`, `user_id`, `created_at` | 좋아요 |
| `share_reports` | `id`, `share_id`, `reporter_id`, `reason`, `status`, `created_at` | 신고 |
...
| 테이블 | 주요 컬럼 | 설명 |
| --- | --- | --- |
| `songs` | `id`, `title`, `artist`, `external_ref`, `metadata`, `status`, `created_by` | 운영자 사전 큐레이션 찬양 메타데이터 |
| `user_song_libraries` | `id`, `user_id`, `song_id`, `created_at` | 사용자의 내 찬양 목록 |
| `simulator_clips` | `id`, `qt_range_id`, `ai_run_id`, `status`, `clip_ref`, `failure_reason`, `updated_at` | 본문 장면 클립 상태 |
...
| 테이블 | 주요 컬럼 | 설명 |
| --- | --- | --- |
| `admin_audit_logs` | `id`, `actor_id`, `actor_type`, `action`, `target_type`, `target_id`, `summary`, `created_at` | 관리자·SYSTEM 작업 감사 로그 |

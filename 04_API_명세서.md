# 📖 QT-AI (큐티 AI 앱) — API 명세서 v1.6

> **문서 버전:** v1.6
> **작성일:** 2026-05-06 (v1.0) / 2026-05-06 (v1.1) / 2026-05-07 (v1.2) / 2026-05-09 (v1.3 — yaml 이슈 6종 패치) / 2026-05-09 (v1.4 — yaml↔명세서 정합) / 2026-05-09 (v1.5 — DECISIONS.md 정합 + journal yaml 재작성) / 2026-05-12 (v1.6 — Journal API 폐기 + Bible Service 통합)
...
| 7.6 | GET | `/api/v1/journals/{id}/events` | ✅ | 이벤트 로그 조회 (감사용) |

> **주의:** **`POST /api/v1/journals` (수동 생성) 없음 v1.0** — 묵상 노트는 AI 세션 완료(`ai.session.completed` Kafka 컨슈머)를 Bible Service가 구독하여 자동 DRAFT 생성한다. 사용자는 **수정·발행만** 가능.
...
### 7.6 GET /api/v1/journals/{id}/events
...
| 이벤트 | 토픽 | 시점 | 멱등성 키 |
| --- | --- | --- | --- |
| `journal.created` | `journal.created` | Bible Service의 AI 세션 컨슈머가 DRAFT 생성 시 (AFTER_COMMIT) | `journal.created:{journalId}` |
...
| 토픽 | Consumer Group | 동작 |
| --- | --- | --- |
| `ai.session.completed` | `bible-service-ai-completion-consumer` | DRAFT Journal + JOURNAL_EVENTS (CREATED) 적재 |
...
**구현:** UseCase 1개 (`GetDashboardUseCase`) — Bible/AI/BFF 정적 매핑 병렬. 어느 하나가 실패해도 partial 응답 (해당 필드만 null + 헤더 `X-Partial-Response: bible-service-down`).

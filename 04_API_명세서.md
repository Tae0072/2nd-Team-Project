...
> **연관 문서:** `00_문서_역할_분리표.md`, `01_프로젝트_계획서.md`, `03_아키텍처_정의서.md`, `05_요구사항_정의서.md`, `18_코드_품질_게이트.md`, `23_도메인_용어사전.md`
...
| 버전 | 날짜 | 작성자 | 주요 변경 |
| --- | --- | --- | --- |
| v0.1 | 2026-05-15 | Codex | `05_요구사항_정의서.md` v2.3 기준으로 API 명세서 초안 작성 |
...
| --- | --- | --- |
| 외부 공개 API | `/api/v1/**` 경로, 메서드, 권한, 요청/응답 요약 | 기능 요구사항 원본은 `05_요구사항_정의서.md` |
| 내부 도메인 인터페이스 | Port 이름, 호출 주체, 제공 도메인, 목적 | 도메인 경계 상세는 `03_아키텍처_정의서.md` |
| 에러 응답 | 공통 에러 포맷, 주요 상태 코드 | 품질 검증은 `18_코드_품질_게이트.md` |
| 금지 API | 사용자용 AI Q&A, SSE, 교회 인증 API 제외 기준 | 제외 근거는 `05_요구사항_정의서.md` |

문서 간 충돌이 발생하면 기능 포함 여부와 사용자 노출 기준은 `05_요구사항_정의서.md` v2.3을 우선한다.
...
| 순서 | 작업 | 목적 |
| --- | --- | --- |
| 1 | 전체 문서 정합성 최종 점검 | 분리 문서 전체가 `05` v2.3과 충돌하지 않는지 확인 |
| 2 | 변경분 커밋/푸시 또는 구현 저장소 반영 준비 | 문서 기준 틀을 공유 가능한 상태로 정리 |
| 보류 | `05_요구사항_정의서.md` 슬림화 | 사용자 요청 전까지 진행하지 않음 |
...
| 협업 규칙 | `14_팀_협업_규칙_Git_브랜치_전략.md` v2.4 재점검 완료 |
| 전체 문서 정합성 최종 점검 | 진행 완료 |
| 실제 `05` 슬림화 | 아직 진행하지 않음 |

# 📖 QT-AI (큐티 AI 앱) — API 명세서 v2.0

> **문서 버전:** v2.0
> **작성일:** … / 2026-05-13 (v1.7) / **2026-05-14 (v2.0 — Modular Monolith·RAG 폐기·sources·EXPLANATIONS 리네이밍)**
> **연관 문서:** [DECISIONS](./DECISIONS.md) / [AGENTS](./AGENTS.md) / [02_ERD_문서 v2.0](./02_ERD_문서.md) / [03_아키텍처_정의서 v2.0](./03_아키텍처_정의서.md) / [09_AI 가이드 v2.0](./09_AI_프롬프트_운영_가이드.md)
> **W1 Lock-in 산출물 (v2.0):** 본 문서 + `apis/{ai,bff,bible}/openapi.yaml` (Spectral lint 통과 + Prism mock 가동). `apis/auth`·`apis/journal`은 폐기 — Auth/Journal API는 각각 `apis/bff` 또는 `apis/bible` 안에 포함.
> **목적:** 6명이 W1 종료 시점에 동일한 API 계약 위에서 병렬로 코드를 작성할 수 있도록 모든 endpoint·DTO·에러·페이로드를 박제.
>
> **2026-05-14 v2.0 갱신 요지:**
> - 백엔드 단일 서비스 `qtai-server`가 도메인별 OpenAPI 3개를 모두 서빙(분할 유지). `service` 컬럼은 "도메인" 의미로 해석.
> - SSE 이벤트 필드 `sources` → **`sources`** 리네이밍 (의미 = 사용된 `bible_explanations.id` 배열).
> - 해설 API 경로: `/api/v1/explanations/commentary/*` → **`/api/v1/explanations/commentary/*`**. 응답 필드명 `commentaries` → `explanations`. DB 테이블 `COMMENTARIES` → `bible_explanations`.
> - `apis/auth/openapi.yaml`·`apis/journal/openapi.yaml` 폐기 확정(5/12·5/14 양 결정).
> - Kafka 토픽은 v2 분리 시 도입 — v1에서는 in-process 이벤트로 처리, envelope·페이로드 구조는 동일 유지.
...
| v1.3 | 2026-05-09 | 강태오 | **yaml 이슈 6종 패치** — bible/ai/journal Prism mock 포트 누락(4012/4013/4014) / ProblemDetail `properties:` 중복 제거 / 에러 응답 example 필수필드 보완(`type`·`title`·`status`·`timestamp`) / bible description 개역개정→개역한글 저작권 오류 수정 / AI Session `status` 불일치(ACTIVE vs IN_PROGRESS) 해소 / `currentStage`↔`guideStep` 필드명 통일 |
| v1.4 | 2026-05-09 | 강태오 | **yaml-명세서 정합 패치** — bible yaml 전면 재작성 (kr/en/commentary 경로 일치, lang param 방식 제거) / ai yaml 전면 재작성 (turns endpoint, IN_PROGRESS status, guideStep, Python FastAPI 명시, advanceStage 제거, 에러코드 보완) / bff yaml 1번 오수정 롤백 (guideStep/IN_PROGRESS 복원) / 03번 §1.3.1 commentary 경로 누락(/api/v1/) 수정 |
...
| v1.6 | 2026-05-12 | 강태오 | **Journal API 폐기 정합 패치** — 별도 `apis/journal/openapi.yaml` 제거 / 묵상 기록 API를 Bible Service OpenAPI(`apis/bible/openapi.yaml`)로 통합 / Gateway journal route target을 bible-service로 변경 |
| v1.7 | 2026-05-13 | Codex | **오전 회의 결정 반영** — 오늘 QT 1구절 기준 / AI·묵상은 오늘 QT만 / `/api/v1/journals/today` 추가 / AI 완료는 기존 오늘 Journal에 요약 첨부 / 찬양·교회 인증 MVP 범위 정리 |
| **v2.0** | **2026-05-14** | 강태오 | **Modular Monolith·RAG 폐기 정합 패치.** 단일 서비스 `qtai-server`가 도메인별 OpenAPI 3개(`apis/{ai,bff,bible}`) 모두 서빙. `apis/auth`·`apis/journal` 폐기 확정. SSE `sources` → `sources` 리네이밍. 해설 경로 `/api/v1/explanations/commentary/*` → `/api/v1/explanations/commentary/*`. Kafka 토픽 표는 "v2 분리 시 도입"으로 표기, v1은 in-process 이벤트. |
...
| AI | `SESSION_ALREADY_COMPLETED` ⭐ | 422 | 이미 묵상 완료 처리된 세션에 turn 추가 시도 |
| AI | `LLM_PROVIDER_ERROR` | 502 | DeepSeek API 5xx |
...
| Share | `SHARE_NOT_FOUND` | 404 | 익명 나눔 없음 |
| Share | `SHARE_ALREADY_EXISTS` | 409 | 이미 공개된 묵상 노트 |
| Share | `SHARE_ALREADY_LIKED` | 409 | 이미 좋아요 처리됨 |
...
| Gateway Auth `/auth/login` | 5 req/min | client IP (brute-force 방지) |
| Gateway Auth `/auth/register` | 3 req/min | client IP |
...
| --- | --- | --- | --- | --- | --- |
| auth-public | `/auth/register` | POST | gateway | ❌ | Auth Service 없음, Gateway가 처리 |
| auth-public | `/auth/login` | POST | gateway | ❌ | Rate 5/min |
| auth-public | `/auth/refresh` | POST | gateway | ❌ | |
| auth-public | `/auth/logout` | POST | gateway | ❌ ⭐ | refresh token만 검증 |
| auth-public | `/auth/oauth/google` | POST | gateway | ❌ | |
| auth-private | `/auth/me` | GET | gateway | ✅ | |
| bible | `/bible/**` | GET | bible-service | ❌ | 성경 본문·목록은 비로그인 허용 |
| bible-explanation | `/api/v1/explanations/**` | GET | bible-service | ❌ | 쉬운 본문 설명 선택 열람 |
| bible-aggregate | `/api/v1/passages/{book}/{ch}/{v}` ⭐ | GET | **bff-aggregator** | ❌/✅ | 한/영 본문은 공개, 주석·개인화는 인증 필요 |
| today-qt | `/api/v1/qt/today` | GET | **bff-aggregator** | ❌/✅ | 오늘의 QT 미리보기 |
| commentary | `/api/v1/explanations/commentary/**` | GET | bible-service | ✅ | 직접 호출 |
| ai | `/ai/sessions` | POST | ai-service | ✅ | |
| ai-sse | `/ai/sessions/*/turns` | POST | ai-service | ✅ | **SSE 패스스루** |
| ai-complete | `/ai/sessions/*/complete` | POST | ai-service | ✅ | 사용자가 "묵상 완료" 선택 |
...
> **Owner:** 강태오 / **Base URL:** Gateway 자체 / **Public:** `https://api.qtai.app/auth/...`
>
> **결정:** 2026-05-12 회의 후 Auth Service는 독립 서비스에서 제거했다. JWT 발급·검증·Google OAuth 진입점은 Gateway에서 처리한다. 별도 `auth-service` 라우팅이나 배포를 생성하지 않는다.
> **소프트 로그인:** 앱 첫 진입에서는 로그인하지 않아도 튜토리얼, 성경 본문, 오늘의 QT 미리보기를 볼 수 있다. 묵상 기록 저장, 주석 열람, AI 질문, 찬양 저장·공유부터 로그인 유도.
...
| 4.7 | POST | `/auth/oauth/google` | ❌ | Google OAuth 로그인 (JWK 직접 검증) |

> 계정 탈퇴 API는 MVP 제외. `user.deactivated` 이벤트도 발행하지 않는다.
...
### 4.7 POST /auth/me/deactivate — MVP 제외

계정 탈퇴 기능은 2026-05-12 회의 후 MVP에서 제외한다. 따라서 `/auth/me/deactivate` API와 `user.deactivated` Kafka 이벤트는 v1.0에서 구현하지 않는다.
### 4.7 POST /auth/oauth/google — JWK 직접 검증 (v1.1)
...
Gateway Auth는 MVP에서 Kafka 이벤트를 발행하지 않는다. 계정 탈퇴와 `user.deactivated` 이벤트는 v1.1 이후 검토한다.

---

## 5. Bible Service API

> **Owner:** 이지윤·이승욱 / **Base URL:** `http://bible-service.qtai.svc.cluster.local:8080`
>
...
| 5.4 | GET | `/api/v1/explanations/commentary/{bookCode}/{chapter}/{verse}` | ✅ | 주석 (Matthew Henry PD + 더미한글) |
| 5.5 | GET | `/api/v1/explanations/{bookCode}/{chapter}/{verse}` | ❌ | 쉬운 본문 설명 선택 열람 |
| 5.6 | GET | `/bible/books` | ❌ | 책 목록 조회 (66권) |
...
### 5.4 GET /api/v1/explanations/commentary/{bookCode}/{chapter}/{verse}
...
StartSessionRequest:
  type: object
  required: [qtDate, passage]
  properties:
    qtDate:
      type: string
      format: date
      description: 오늘 QT 날짜. MVP에서는 서버의 오늘 QT와 일치해야 한다.
...
> **MVP 범위:** AI 세션은 **오늘 QT 본문에서만** 시작할 수 있다. 사용자가 성경 검색/일반 본문 화면에서 임의 구절로 AI를 시작하면 `422 AI_PASSAGE_NOT_TODAY_QT`를 반환한다.
...
- `sources` — 응답 후반부에 사전 적재 해설 row 출처 (매 ASSISTANT 턴마다 필수 — 03번 § 3.3 신학 가드레일)
...
- 전체 max: 5분
- 클라이언트 연결 끊김 → `Flux.doFinally(SignalType.CANCEL)` → DeepSeek 호출 즉시 중단 (토큰 비용 절약)
...
- 422 `SESSION_ALREADY_COMPLETED` — 이미 묵상 완료된 세션
...
```
event: error
data: {"code": "LLM_PROVIDER_ERROR", "detail": "DeepSeek API 503 after 3 retries", "traceId": "..."}
...
| 이벤트 | 토픽 | 시점 | 멱등성 키 |
| --- | --- | --- | --- |
| `ai.session.completed` | `ai.session.completed` | 사용자가 "묵상 완료" 선택 시 (AFTER_COMMIT) | `ai.session.completed:{sessionId}` |
...
## 7. Bible Service 묵상 기록 API

> **Owner:** 이지윤·이승욱 / **Base URL:** `http://bible-service.qtai.svc.cluster.local:8080`
>
> **OpenAPI 파일:** `apis/bible/openapi.yaml`
>
> **중요:** 별도 Journal API/OpenAPI는 사용하지 않는다. `/api/v1/journals...` 경로는 Bible Service가 소유한다.

### 7.1 Endpoint 요약

| # | Method | Path | 인증 | 설명 |
| --- | --- | --- | --- | --- |
| 7.2 | POST | `/api/v1/journals/today` | ✅ | 오늘 QT journal DRAFT 생성 또는 기존 DRAFT 조회 |
| 7.3 | GET | `/api/v1/journals` | ✅ | 본인 journal 목록 (페이지네이션) |
| 7.4 | GET | `/api/v1/journals/{id}` | ✅ | journal 단건 조회 |
| 7.5 | PATCH | `/api/v1/journals/{id}` | ✅ | journal 자동 저장 수정 (이벤트 소싱) |
| 7.6 | DELETE | `/api/v1/journals/{id}` | ✅ | journal 삭제 (Soft Delete) |
| 7.7 | GET | `/api/v1/journals/{id}/events` | ✅ | 이벤트 로그 조회 (감사용) |

> **주의:** **`POST /api/v1/journals` (자유 본문 수동 생성) 없음 v1.0**. MVP 묵상 노트는 오늘 QT 본문에만 연결된다. Flutter가 오늘 QT 화면에 진입하거나 사용자가 기록을 시작할 때 `POST /api/v1/journals/today`를 호출해 DRAFT를 확보한다. AI 세션 완료(`ai.session.completed`)는 새 Journal을 만들지 않고 같은 `qtDate + userId` Journal에 AI 요약·세션 연결만 반영한다.

### 7.2 POST /api/v1/journals/today

오늘 QT 본문에 연결된 본인 Journal DRAFT를 idempotent하게 가져온다. 이미 오늘 Journal이 있으면 `200 OK`, 없으면 `201 Created`.

**Request:** 본문 없음. 날짜는 서버 기준 오늘 QT(`Asia/Seoul`)를 사용한다.
...
**Errors:**
- 404 `TODAY_QT_NOT_FOUND` — 운영자가 오늘 QT 본문을 아직 등록하지 않음
- 409 `JOURNAL_LOCKED` — 같은 사용자 동시 생성 경쟁. 클라이언트는 1회 재조회

### 7.3 GET /api/v1/journals
...
> **본인 journal만** (X-User-Id 자동 필터). soft-deleted (`deleted_at IS NOT NULL`)는 기본 제외. `PageResponse<T>` (§ 2.7). MVP에서는 `qtDate` 기준 일자별 1개 Journal을 기본 단위로 삼는다.

### 7.4 GET /api/v1/journals/{id}
...
```json
{
  ...
  "title":        "태초의 묵상",
  "felt":         "오늘 묵상에서 깨달은 것은...",
  "memorableVerse": "창세기 1:1",
  "application":  "오늘 하루의 시작을 기도로 열기",
  "prayer":       "창조주 하나님을 기억하게 해주세요.",
  "status":       "PUBLISHED",
  "aiSessionId":  9012,
  "lastEventSequence": 4,
  "createdAt":    "2026-05-26T14:45:00Z",
...
### 7.5 PATCH /api/v1/journals/{id} — 자동 저장 수정 (이벤트 소싱)

Flutter는 별도 저장 버튼 없이 입력 debounce 후 PATCH한다. 사용자에게 보이는 글자 수 제한은 두지 않는다. 서버는 공통 request size, DB 타입, 악성 payload 방어만 적용한다.
...
| 이벤트 | 토픽 | 시점 | 멱등성 키 |
| --- | --- | --- | --- |
| `journal.created` | `journal.created` | `POST /api/v1/journals/today`가 오늘 QT DRAFT를 최초 생성할 때 (AFTER_COMMIT) | `journal.created:{journalId}` |
...
| 토픽 | Consumer Group | 동작 |
| --- | --- | --- |
| `ai.session.completed` | `bible-service-ai-completion-consumer` | 오늘 QT Journal에 `aiSessionId`·AI 요약을 첨부하고 JOURNAL_EVENTS(UPDATED) 적재 |
| `user.activity.tracked` | `bible-service-activity-stats-consumer` | 구절 조회 이력·통계 적재 |
...
| 8.3 | GET | `/api/v1/passages/{bookCode}/{chapter}/{verse}` | ✅ | 읽기 전용 성경 화면 (Bible 3개 endpoint 병렬) |
| 8.4 | WS | `/ws/notifications` | ✅ | 실시간 알림 (STOMP) |

### 8.2 GET /api/v1/me/dashboard — v1.1 호출 service 정확화

**처리:** **Gateway 사용자 컨텍스트 + 3개 작업 병렬 호출** (`CompletableFuture.allOf`):
1. **Gateway 인증 컨텍스트** → `user` (id, nickname)
2. **Bible Service** `/api/v1/journals?size=5&sort=createdAt,desc` → `recentJournals` + `stats.journalCount`
...
4. **BFF 자체 오늘 QT 매핑** → `todayPassage` (MVP는 하루 1구절. `verseStart`와 `verseEnd`는 같은 값)
...
> **stats.currentStreak / longestStreak:** v1.0은 Bible Service가 본인 journal 일자별 집계로 계산 (간단 SQL). v1.1에 별도 `user.activity.tracked` Kafka 컨슈머로 통계 테이블 분리.
...
**구현:** UseCase 1개 (`GetDashboardUseCase`) — 오늘 QT 데이터는 첫 화면 진입을 위해 우선 반환하고, 최근 journal/activeSession 등 부가 데이터는 병렬 로딩한다. 어느 하나가 실패해도 partial 응답 (해당 필드만 null + 헤더 `X-Partial-Response: bible-service-down`).

### 8.3 GET /api/v1/passages/{bookCode}/{chapter}/{verse}

> 일반 성경 본문/검색 화면은 MVP에서 **읽기 전용**이다. 이 경로로 조회한 임의 구절에서는 AI 세션 시작과 Journal 생성 액션을 노출하지 않는다.

**처리 (v1.1 명시):** 03번 § 1.3.1 시나리오 1. **BFF가 Bible Service의 3개 endpoint를 병렬 직접 호출** (단일 책임 + 캐시 단위 명확):
1. `GET /bible/kr/{...}` → `kr`
2. `GET /bible/en/{...}` → `en`
3. `GET /api/v1/explanations/commentary/{...}` → `commentaries`
...
| `sources` | `{ "sources": [{ "type": ..., "id": ..., "weight": ... }] }` | 1회 | **ASSISTANT 턴마다 필수** (신학 가드레일) |
...
| 클라이언트 끊김 | `Flux.doFinally(SignalType.CANCEL)` → DeepSeek 호출 즉시 dispose |
...
| --- | --- | --- |
| `JOURNAL_AI_DRAFT_READY` | AI 세션 완료 → 오늘 Journal에 AI 요약 첨부 | `journalId`, `aiSessionId` |
...
openapi: 3.1.0
info:
  title: QT-AI Bible Service
  version: 1.0.0
  description: |
    성경 본문·쉬운 설명·주석·묵상 기록·익명 나눔 Service.
    Owner: 이지윤·이승욱
  contact:                          # ⭐ v1.1 — Spectral 룰 info-contact: error 통과용
    name: 이지윤·이승욱 (Bible Service Owners)
servers:
  - url: https://api.qtai.app
    description: Production (via Gateway)
  - url: http://localhost:4012        # ⭐ v1.1 — § 12.2 포트와 일관
    description: Local Prism mock (bible)
...
| 요일 | 산출물 |
| --- | --- |
| 월 5/18 | OpenAPI 골격 4개 작성 (operationId만) |
| 화 5/19 | request·response schema + example 작성 → **Spectral lint 통과** |
| 화 5/19 ~ | Prism mock 4포트 가동 → Flutter `dev` 환경 연결 → 김지민 코딩 시작 |
...
prism proxy apis/bible/openapi.yaml http://bible-service:8080 --port 4012 --errors
...
```bash
npm install -g dredd
dredd apis/bible/openapi.yaml http://bible-service:8080 --hookfiles=./hooks.js
...
| --- | --- | --- |
| API 응답 일관성 부재 (envelope 혼용) | 4 service × envelope 종류 → 클라이언트 분기 폭증 | § 2.3 envelope 사용 안 함 + § 2.4 ProblemDetail 표준화 |
| 에러 응답 형식 제각각 | 4 service × 에러 형식 환각 | § 2.4 RFC 7807 ProblemDetail + § 2.6 도메인 코드 표 (28개) + Spectral 룰로 강제 |
| HTTP status 잘못 사용 (200 + success:false) | 4 service × status 잘못 | § 2.5 표 + 머지 리뷰 체크 + § 2.5 503 매핑 책임 명시 |
| 인증 헤더 직접 검증 누락 | 4 service × 인증 누락 가능 | § 2.1 Gateway 단일 검증 + ADR-0012 |
| 페이지네이션 형식 제각각 | 4 service × 다른 응답 | § 2.7 Custom `PageResponse<T>` 표준 |
...
[x] § 6  AI API 4 endpoints + SSE 7개 event + guideStep schema default + SYSTEM 마스킹 schema + 컨슈머 group
[x] § 7  Bible 묵상 기록 API 5 endpoints + idempotency_key 자동 생성 + events 02번 정합 + 컨슈머 group
[x] § 8  BFF API 3 endpoints + Gateway 사용자 컨텍스트 + 3개 작업 병렬 + activeSession 정책 + epoch_minute 분 경계
...
- [ ] `apis/{service}/openapi.yaml` 4개 모두 존재 (auth/bible/ai/bff)
- [ ] Spectral lint 4개 모두 통과 (CI 그린)
- [ ] **info.contact** 4개 모두 작성 — Spectral `info-contact: error` 통과
- [ ] **모든 4xx/5xx 응답에 `application/problem+json`** 명시 — Spectral `problem-json-on-errors: error` 통과
- [ ] Prism mock v5+ 4 포트 동시 가동 가능 — `prism mock apis/bff/openapi.yaml --port 4015` 응답 200
- [ ] Flutter `dev` 환경에서 BFF mock에 GET /api/v1/me/dashboard 호출 → 본 문서 § 8.2 example 응답 수신
- [ ] **`PageResponse<T>` Custom DTO** 공통 module에 1개 박제 — Bible 묵상 기록·AI 응답 검증
- [ ] 4개 OpenAPI spec의 모든 endpoint에 operationId + summary 있음
...
15. § 7.4 Bible 묵상 기록 PATCH idempotency_key — 서버 자동 생성 명시 (`journal.update:{ULID}`)
16. § 6.2 guideStep — OpenAPI schema `default: A` 명시
...
**🟢 선택적 개선 12개**
25. § 1.4 Slack 채널 (#qtai-api / #qtai-deploy / #qtai-incident) — W0 5/13까
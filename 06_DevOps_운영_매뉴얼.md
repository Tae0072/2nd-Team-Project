📋 QT-AI (큐티 AI 앱) — DevOps 운영 매뉴얼 v2.0

> **문서 버전:** v2.0
> **작성일:** 2026-05-07 / **2026-05-14 (v2.0 — Modular Monolith·Docker Compose, K8s/Helm v2 부록)**
> **연관 문서:** [DECISIONS](./DECISIONS.md) / [AGENTS](./AGENTS.md) / [02_ERD_문서 v2.0](./02_ERD_문서.md) / [03_아키텍처_정의서 v2.0](./03_아키텍처_정의서.md)
> **DevOps 키워드 (v2.0):** GitHub Actions · Docker buildx · ghcr.io · **Docker Compose** · Flyway · Loki/Prometheus/Jaeger · Rollback · Incident Runbook · 온콜 (v2 분리 시: Helm · Minikube · K8s)
> **W1 Lock-in 산출물 (v2.0):** 본 문서 + `.github/workflows/ci.yml` + `docker-compose.yml` + Grafana 대시보드 JSON
> **목적:** 03번 아키텍처 청사진을 **실행 가능한 운영 절차**로 변환.
>
> **2026-05-14 v2.0 갱신 요지:**
> - 배포 단위: 단일 `qtai-server` 컨테이너 + MySQL + Redis. Docker Compose 단일 명령.
> - K8s/Helm/Minikube 자료(§5~§9의 대부분)는 v2 분리 시 자료로 보존. v1 운영은 Docker Compose로.
> - `helm/qtai-umbrella/`·`helm/qtai-infra/`도 v2 자료로 보존.
> - 시크릿: `.env` (개발) / OS env (시연). K8s Secret는 v2에서.
> - 본문 K8s·Helm 절은 상충하는 부분이 있으면 위 v2.0 요지를 우선.
...
| **Least Surprise** | 배포 절차·rollback 절차 4 service 동일. 한 번 익히면 다 됨 |
...
jobs:
  ...
  image:
    ...
    strategy:
      matrix:
        service: [gateway, bff-aggregator, bible-service, ai-service]
    steps:
      - uses: actions/checkout@v4
  ...
  deploy-dev:
    ...
    steps:
      ...
      - name: 배포 검증 (smoke test)
        run: |
          # /actuator/health 4 service 모두 통과 확인
          for svc in gateway bff-aggregator bible-service ai-service; do
...
> **금지:** DeepSeek API key·Google OAuth·DB 비밀번호는 GitHub Secrets에 두지 않음 — K8s Secret만 (05번 § 5.4). GitHub Secrets는 CI/CD 자체 운영용만.
...
```dockerfile
# {service}/Dockerfile (4 service 모두 동일 패턴)
...
```
helm/
├── qtai-umbrella/                         # 4 service umbrella chart
...
│       ├── bible-deployment.yaml
│       ├── bible-service.yaml
│       ├── bible-networkpolicy.yaml
│       ├── ai-deployment.yaml
│       ├── ai-service.yaml
│       ├── ai-networkpolicy.yaml
...
services:
  ...
  ai-service:
    env:
      LOG_LEVEL: DEBUG
      DEEPSEEK_MODEL: deepseek-chat        # dev는 비용을 고려해 환경 변수로 모델 고정
```
...
metadata:
  ...
  labels:
    {{- include "qtai.labels" (dict "name" "gateway" "Release" .Release "Chart" .Chart "Values" .Values) | nindent 4 }}
spec:
...
[4] Smoke test (자동)
    └─ /actuator/health 4 service 200 OK
...
kubectl logs -f deployment/gateway -n qtai
...
> **중요:** 모든 service 간 호출·Kafka publish/consume·DeepSeek 호출은 모두 trace에 포함. 사용자 신고 시 `trace_id` (04번 § 2.4 ProblemDetail)로 즉시 조회.
...
| 대시보드 | 지표 |
| --- | --- |
| **QT-AI Overview** | 4 service up/down, request rate, error rate, p95 latency |
...
| 5. DeepSeek API 5xx 다발 | P1 | 1h | § 10.5 |
...
```bash
# 1. 로그 확인
kubectl logs -n qtai pod/gateway-xxxx --previous   # 직전 crash 로그

# 2. 원인별 분기
# 2-1) OOMKilled
kubectl describe pod gateway-xxxx -n qtai | grep -i killed
...
kubectl exec -it kafka-0 -n qtai -- \
  kafka-consumer-groups.sh --bootstrap-server localhost:9092 \
    --describe --group bible-service-ai-completion-consumer

# 2. consumer 단위 분석
# 2-1) consumer Pod 다운 → § 10.3
# 2-2) consumer 처리 시간 ↑ → 코드 hotfix
# 2-3) 단순 backlog → consumer Pod replicas 증가
kubectl scale deployment bible-service --replicas=2 -n qtai
...
| --- | --- | --- |
| **시연 직전 부랴부랴 배포** (운영 환경 부재) | 4 service × 인프라 4 = 10 컴포넌트 동시 배포 → 폭증 | § 3 GHA 5단계 + § 5 Helm Chart + § 6 매주 정기 배포 W2부터 |
...
| **로그 없이 디버깅** | 4 service 로그 흩어짐 | § 8.2 Loki + Promtail 라벨링 |
| **메트릭 없이 운영** | 어느 service가 느린지 모름 | § 8.3 Prometheus + alert 11개 룰 |
| **trace 없이 분산 디버깅** | 4 service 호출 흐름 불투명 | § 8.4 Jaeger + W3C Trace Context |
...
- [ ] 4 service Dockerfile 표준 (§ 4.1) 동일 패턴 적용
...
- [ ] `helm/qtai-umbrella/templates/` 디렉토리에 4 service deployment + service + networkpolicy yaml
...
- [ ] 4 service `/actuator/health` 통과 + Helm 첫 배포 성공
...
- `helm/qtai-umbrella/` (4 service deployment + service + networkpolicy)

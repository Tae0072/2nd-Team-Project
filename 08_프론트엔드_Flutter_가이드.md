...
> **문서 버전:** v1.1
> **작성일:** 2026-05-07 (v1.0) / 2026-05-13 (v1.1 — 오전 회의 MVP 범위 반영)
> **연관 문서:** [01_프로젝트_계획서 v1.4](./01_프로젝트_계획서.md) / [02_ERD_문서 v1.6](./02_ERD_문서.md) / [03_아키텍처_정의서 v1.3](./03_아키텍처_정의서.md) / [04_API_명세서 v1.7](./04_API_명세서.md) / [05_보안_명세서 v1.0](./05_보안_명세서.md) / [06_DevOps_운영_매뉴얼 v1.0](./06_DevOps_운영_매뉴얼.md) / [07_테스트_전략 v1.0](./07_테스트_전략.md) / [24_회의결정_2026-05-13_오전](./24_회의결정_2026-05-13_오전.md)
...
| v1.1 | 2026-05-13 | Codex | 오전 회의 결정 반영 — 앱 첫 화면 오늘 QT / 별도 홈 제외 / AI·묵상은 오늘 QT만 / Journal 자동 저장 / 찬양 추천 저장·제거 / 교회 인증 MVP 기본 제외 |
...
8. [SSE 클라이언트 — 오늘 QT AI 스트리밍](#8-sse-클라이언트--오늘-qt-ai-스트리밍)
...
- 5 핵심 화면 (오늘 QT·읽기 전용 성경·AI 큐티·묵상 노트 목록·노트 작성/편집)
...
### 1.1.1 2026-05-13 오전 회의 MVP UX 결정

김지민은 아래 결정을 화면 설계의 최상위 제약으로 둔다.

- 앱 시작 후 별도 홈 화면 없이 **오늘 QT 화면(`/today`)**으로 바로 진입한다.
- 오늘 QT 데이터(본문 1구절, 쉬운 설명 요약/배경/어려운 단어/출처)를 먼저 로딩하고, 최근 묵상·AI 세션·성경 목록 같은 부가 데이터는 백그라운드로 로딩한다.
- 일반 성경 본문/검색 화면은 읽기 전용이다. AI 시작·묵상 작성 버튼은 오늘 QT 본문에서만 노출한다.
- 묵상 기록은 `POST /api/v1/journals/today`로 오늘 DRAFT를 확보한 뒤 4개 필드(`felt`, `memorableVerse`, `application`, `prayer`)를 자동 저장한다. 별도 저장 버튼과 사용자 노출 글자 수 제한은 두지 않는다.
- AI 질문은 오늘 QT 본문에 대한 1회성 응답 중심이다. 응답 말투는 차분한 정보 전달형으로 표시하고, AI 응답은 저장된다는 전제를 UI에 반영한다.
- 찬양은 AI가 오늘 QT 주제 기반으로 추천한 곡을 사용자가 저장/제거하는 흐름만 MVP에 포함한다. 사용자가 YouTube URL을 직접 입력하는 기능은 제외한다.
- 교회 인증은 MVP 기본 기능이 아니다. 회원가입/프로필에 인증 버튼 자리를 둘 수 있지만, 교회 인증이 없어도 앱 사용과 커뮤니티 접근을 막지 않는다.
...
| 인증 토큰·secure_storage·Refresh 흐름 | 김지민 | 강태오 (Gateway JWT/OAuth API 계약 합의) |
...
│   │   ├── today_qt/
│   │   │   ├── data/today_qt_api.dart    # Retrofit (BFF /me/dashboard + journal today)
│   │   │   ├── domain/today_qt.dart
│   │   │   └── presentation/today_qt_page.dart
│   │   ├── passage/                      # 입체 묵상
│   │   │   ├── data/passage_api.dart     # Retrofit (BFF /passages/...)
│   │   │   ├── domain/
│   │   │   └── presentation/passage_view_page.dart
│   │   ├── ai_session/                   # 오늘 QT AI 질문
│   │   │   ├── data/
...
| TodayQtPage | `todayQtProvider` (FutureProvider) | 오늘 QT 본문 우선 호출 결과 |
...
GoRouter appRouter(AppRouterRef ref) {
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      final isPublicReadOnly = state.matchedLocation == '/today'
          || state.matchedLocation.startsWith('/passages');
      final isAuthenticated = auth.value != null;

      if (!isAuthenticated && !isLoggingIn && !isPublicReadOnly) return '/login';
      if (isAuthenticated && isLoggingIn) return '/today';
      return null;
    },
    routes: [
      ...
      GoRoute(
        path: '/today',
        builder: (_, __) => const TodayQtPage(),
        routes: [
          GoRoute(
...
| 시나리오 | 메서드 | 이유 |
| --- | --- | --- |
| 로그인 성공 → 오늘 QT | `context.go('/today')` | back으로 로그인 화면 못 돌아오게 |
| 로그아웃 → 로그인 | `context.go('/login')` | 동일 |
| 오늘 QT → 묵상 노트 목록 | `context.push('/journals')` | back으로 오늘 QT 돌아올 수 있게 |
| 오늘 QT → Journal 자동 저장 화면 | `context.push('/journals/$journalId/edit')` | `POST /journals/today` 후 기존 DRAFT 편집 |
| 알림 클릭 → Journal 상세 | `context.push('/journals/$journalId')` | back으로 이전 화면 |
...
| 2 | TodayQtPage | `/today` | `todayQtProvider` + `journalTodayProvider` | `/me/dashboard`, `/journals/today` |
| 3 | PassageViewPage (읽기 전용 성경) | `/today/passage/:bookCode/:chapter/:verse` | `passageProvider(family)` | `/passages/:b/:c/:v` (BFF) |
| 4 | AiSessionPage (오늘 QT AI 질문) | `/ai/sessions/:sessionId` | `aiSessionTurnsProvider(sessionId)` | `/ai/sessions/:id`, `/ai/sessions/:id/turns` (SSE), `/ai/sessions/:id/abandon` |
| 5a | JournalListPage | `/journals` | `journalListProvider` + `journalFilterProvider` | `/journals?page=&size=&status=` |
| 5b | JournalDetailPage / JournalEditPage | `/journals/:id`, `/journals/:id/edit` | feature 내 단일 Provider | `/journals/today` POST, `/journals/:id` GET·PATCH·DELETE |
...
### 10.3 화면 2 — TodayQtPage (첫 화면)

```dart
// lib/features/today_qt/presentation/today_qt_page.dart
class TodayQtPage extends ConsumerWidget {
  const TodayQtPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final asyncToday = ref.watch(todayQtProvider);
    ...
    return Scaffold(
      ...
      body: asyncToday.when(
        ...
        error: (e, _) => ErrorView(error: e, onRetry: () => ref.invalidate(todayQtProvider)),
        data: (dashboard) => RefreshIndicator(
          onRefresh: () async => ref.invalidate(todayQtProvider),
          child: ListView(
            padding: const EdgeInsets.all(16),
            children: [
              // 오늘 QT 본문 1구절. verseStart == verseEnd.
              _PassageCard(verse: dashboard.todayPassage),
              const SizedBox(height: 16),

              // 쉬운 설명은 DB 저장 본문: 요약, 배경, 어려운 단어, 출처.
              _ExplanationPanel(explanation: dashboard.explanation),
              const SizedBox(height: 16),

              // 로그인 필요 액션. 누르면 /journals/today로 DRAFT 확보.
              _TodayJournalActions(qtDate: dashboard.todayPassage.qtDate),
              const SizedBox(height: 16),

              // AI 질문은 오늘 QT 본문에서만 시작.
              _AiQuestionCard(passage: dashboard.todayPassage),
            ],
          ),
        ),
      ),
...
class _PassageCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      child: InkWell(
        onTap: () => context.push(
          '/today/passage/${verse.bookCode}/${verse.chapter}/${verse.verse}',
        ),
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Column(
            ...
            children: [
              Text('오늘 QT', style: Theme.of(context).textTheme.labelMedium),
...
- 첫 화면은 오늘 QT 본문을 가장 먼저 보여주고, 최근 묵상·진행 중 AI 세션·알림 같은 부가 정보는 별도 Provider로 뒤따라 로딩한다.
- AI 시작과 묵상 작성은 오늘 QT 본문에서만 활성화한다. 일반 성경 본문 화면에는 읽기 전용 액션만 둔다.
...
> 이 화면은 읽기 전용이다. AI 큐티 시작과 Journal 생성은 TodayQtPage에서만 제공한다.

### 10.5 화면 4 — AiSessionPage (오늘 QT 1회성 질문)

오늘 QT 본문을 기준으로 1회성 질문과 응답을 처리한다. SSE 스트림 + 사용자 입력 + 응답 저장 상태 표시가 핵심이다.
...
- `_SessionScopeBanner` — 오늘 QT 본문 기준 질문임을 표시
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          final journal = await ref.read(journalRepositoryProvider).createOrGetToday();
          if (context.mounted) context.push('/journals/${journal.id}/edit');
        },
...
JournalEditPage — 자유 본문 신규 생성자는 두지 않는다. 오늘 QT DRAFT는 `POST /api/v1/journals/today`로 확보하고, 이후 `JournalEditPage.edit(journalId: id)`만 사용한다.
...
- 마지막 성공한 TodayQt 데이터를 메모리에 1회 보존 (앱 재시작 시 사라짐) — Riverpod의 `KeepAlive` 옵션 활용
  ...
  group('login', () {
    test('성공 시 token 저장 + User 반환', () async {
      when(api.login(any)).thenAnswer((_) async => TokenPair(
        accessToken: 'AT', refreshToken: 'RT', expiresIn: 1800, refreshExpiresIn: 1209600,
      ));
...
| # | 시나리오 | 검증 포인트 |
| --- | --- | --- |
| 1 | 로그인 → 오늘 QT 진입 | 토큰 저장·BFF /me/dashboard 호출·오늘 QT 본문 표시 |
| 2 | 입체 묵상 화면 진입·스크롤 | 같은 장 절 표시·현재 절 자동 스크롤 |
| 3 | 오늘 QT AI 질문 | SSE 토큰 스트림·turn_completed·세션 완료 |
...
void main() {
  ...
  testWidgets('로그인 → 오늘 QT 진입', (tester) async {
...
- [ ] 5 화면 라우팅 골격 (LoginPage, TodayQtPage, PassageViewPage, AiSessionPage, JournalListPage) — 빈 Scaffold라도 OK
...
**페어 합의:**
- [ ] 김지민 ↔ 강태오: Gateway 인증 API 계약 (login, refresh, oauth/google) — Retrofit 인터페이스로 박제
- [ ] 김지민 ↔ 강상민: AI SSE 이벤트 4종 (token, turn_completed, error, end) — `AiTurnEvent` sealed class로 박제
- [ ] 김지민 ↔ 강태오: 오늘 QT 첫 화면 응답·STOMP CONNECT 헤더 명·SSE Gateway 우회 — 04번 § 8.2 + § 9.6 + § 10.2 정합
...
| **W2 (5/26~29)** | LoginPage·RegisterPage 완성 + TodayQtPage 첫 화면 호출 + JournalListPage 골격 |
...
| W2 화 (5/26) 11:30 | TodayQtPage 호출 동작 | 오늘 QT 본문 먼저 표시 OK |
...
4. **5 화면 중 1개를 잘라낼 수 있는 우선순위** (시연 위해): 필수(Login·TodayQt·AiSession·JournalList) > 선택(PassageView·JournalEdit). PassageView가 가장 어려움 → W4 시점에 미달이면 PassageView 단순화 (Bible Service 단일 호출 → Sliver 없이 단순 ListView)

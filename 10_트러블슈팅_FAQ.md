...
4. [Gateway Auth — JWT·401·Refresh·Google OAuth](#4-gateway--jwt401refreshgoogle-oauth)
5. [AI Service — LLM timeout·환각·ChromaDB·인젝션](#5-ai-service--llm-timeout환각chromadb인젝션)
6. [Bible Service Journal — Kafka·Saga·이벤트 순서](#6-bible-service--kafkasaga이벤트-순서)
...
9. [Gradle 빌드 — 4 module·Kotlin DSL·의존성](#9-gradle-빌드--6-modulekotlin-dsl의존성)
...
```bash
# 1. 백엔드 pod 상태
kubectl get pods -n qtai | grep gateway

# 2. Service endpoint 확인 (NONE이면 selector 불일치)
kubectl get endpoints gateway -n qtai

# 3. Pod 로그 마지막 50줄
kubectl logs deployment/gateway -n qtai --tail=50
...
spec:
  selector:
    app: gateway  # ← Pod의 label과 정확히 일치?
...
3. **서비스 이름** — `lb://gateway` 의 `gateway`가 K8s Service 이름과 일치?
...
## 4. Gateway Auth — JWT·401·Refresh·Google OAuth
...
**빠른 확인:**
```bash
# Gateway Auth 로그 — JWT 검증 오류 grep
kubectl logs deployment/gateway -n qtai --tail=100 | grep -i "jwt\|token\|401\|unauthorized"
...
| Gateway Auth | `/auth/oauth/google` 에서 Google JWK 검증 (04번 § 4.8) — `https://www.googleapis.com/oauth2/v3/certs` 접근 가능한지 |
...
```bash
# Gateway Auth에서 Google JWK 접근 확인
kubectl exec -it deployment/gateway -n qtai -- curl -v \
...
| 원인 | 해결 |
| --- | --- |
| DeepSeek 모델 silently 업데이트 | `DEEPSEEK_MODEL` 환경변수와 09번 § 3.1 기준 확인 |
...
## 6. Bible Service Journal — Kafka·Saga·이벤트 순서
...
# 2. Bible Service Journal consumer lag
kubectl exec -it deployment/kafka -n qtai -- kafka-consumer-groups.sh \
  --bootstrap-server localhost:9092 --group qtai-journal-group --describe

# 3. Bible Service Journal 로그
kubectl logs deployment/bible-service -n qtai --tail=50 | grep -i "journal\|error\|kafka"
...
**AI Service에서 DeepSeek 호출 중인지:**
```bash
kubectl logs deployment/ai-service -n qtai -f | grep "DeepSeek call"
```
...
## 9. Gradle 빌드 — 4 module·Kotlin DSL·의존성
...
| 3 | settings.gradle.kts에 모듈 포함 | `include(":gateway", ":bible-service", ...)` |
...
```bash
# 의존성 트리 확인 (충돌 부분 표시)
./gradlew :gateway:dependencies | grep -A5 "CONFLICT\|---"
...
kubectl run curl-test --image=curlimages/curl --rm -it --restart=Never \
  -n qtai -- curl http://gateway:8080/actuator/health
```
...
# 4. 서비스의 DB 환경 변수 확인
kubectl exec -it deployment/gateway -n qtai -- env | grep "SPRING_DATASOURCE"
```
...
```bash
# Flyway 오류 로그
kubectl logs deployment/gateway -n qtai | grep -i "flyway\|migration"
```
...
# 서비스 재시작
kubectl rollout restart deployment/gateway -n qtai
```
...
# 서비스 Redis 환경 변수 확인
kubectl exec -it deployment/gateway -n qtai -- env | grep REDIS
```
...
docker buildx build --platform linux/amd64 \
  -t gateway:latest ./gateway

# Spring Boot layered jar 빌드 (권장)
./gradlew :gateway:bootBuildImage --imageName=qtai/gateway:latest

# Minikube용 이미지 빌드 (Minikube docker env 사용)
eval $(minikube docker-env)
./gradlew :gateway:bootBuildImage --imageName=gateway:local
...
logging:
  level:
    ...
    com.qtai: DEBUG
  pattern:
    console: '{"timestamp":"%d{ISO8601}","level":"%level","service":"gateway","traceId":"%X{traceId}","message":"%msg"}%n'
```

JSON 형태 구조화 로그여야 Loki에서 `{service="gateway"}` 쿼리로 필터링 가능.
...
| 3 | **평문 시크릿 commit** | `DEEPSEEK_API_KEY`·`MYSQL_PASSWORD` 코드 직접 | 05번 § 2 + § 10.1 | `git log --all -S "sk-"` |
...
| 6 | **타임아웃 미설정** | DeepSeek 호출 무한 대기·앱 멈춤 | § 5.1 + 09번 § 10.3 | DIO timeout·WebClient timeout 확인 |
...
| Gradle 4 module 빌드 | `:gateway:build` 실패 | § 9.1 + `./gradlew :gateway:build --stacktrace` | 강태오 |
...
| DeepSeek API key 설정 | AI Service CrashLoop | § 5.1 → K8s Secret 확인 | 강상민 |

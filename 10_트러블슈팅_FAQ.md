...
> **문서 버전:** v1.0
> **작성일:** 2026-05-08
> **연관 문서:** [01_프로젝트_계획서 v1.3](./01_프로젝트_계획서.md) / [03_아키텍처_정의서 v1.2](./03_아키텍처_정의서.md) / [04_API_명세서 v1.2](./04_API_명세서.md) / [05_보안_명세서 v1.0](./05_보안_명세서.md) / [06_DevOps_운영_매뉴얼 v1.0](./06_DevOps_운영_매뉴얼.md) / [08_프론트엔드_Flutter_가이드 v1.0](./08_프론트엔드_Flutter_가이드.md) / [09_AI_프롬프트_운영_가이드 v1.0](./09_AI_프롬프트_운영_가이드.md)
> **owner:** 강태오 (Lead — 문서 작성 + 관리)
> **용도 키워드:** 디버깅 · 오류 해결 · CrashLoop · 401 · 502 · SSE buffering · Kafka lag · ChromaDB · Flutter DIO · Riverpod · Minikube · Gradle multi-module · PostgreSQL migration · Redis · Loki · Tempo
...
| 버전 | 날짜 | 작성자 | 주요 변경 |
| --- | --- | --- | --- |
| v1.0 | 2026-05-08 | 강태오 (Lead) | 초기 작성 — 16 sections — Gateway/BFF·Auth·AI·Journal·Bible·Flutter·Gradle·K8s·PostgreSQL·Redis·Kafka·CI/CD·관측성·1차 사고 통합 매트릭스·W1 체크리스트 |
...
11. [PostgreSQL·Flyway — 연결·마이그레이션·schema](#11-postgresqlflyway--연결마이그레이션schema)
...
15. [관측성 — Grafana·Prometheus·Tempo·Loki](#15-관측성--grafanaprometheustempeloki)
...
| 1 (DB 연결 실패) | PostgreSQL pod 미시작 or 포트 오류 | `kubectl get pods -n qtai` — DB pod 먼저 확인 |
...
## 11. PostgreSQL·Flyway — 연결·마이그레이션·schema

### 11.1 PostgreSQL 연결 실패
...
## 15. 관측성 — Grafana·Prometheus·Tempo·Loki
...
### 15.2 Tempo trace 없음 (분산 추적)

**증상:** Grafana Explore → Tempo에서 traceId 검색 결과 없음.
...
3. **Tempo pod 상태**:
   ```bash
   kubectl get pods -n qtai | grep tempo
   kubectl logs deployment/tempo -n qtai --tail=30
   ```
...
| PostgreSQL 연결 | 서비스 CrashLoop | § 11.1 → `kubectl describe pod` | 강태오 |

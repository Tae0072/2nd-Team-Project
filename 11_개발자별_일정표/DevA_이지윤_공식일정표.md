...
> **기준일: 2026-05-14 / 기준 결정: 2026-05-14 오전 회의 (Modular Monolith 전환 + Bible팀 3인 재배치)**
>
> **2026-05-14 v2.0 변경 요지 (본 일정표는 W1 첫날 본인 손으로 본문 갱신 예정. 상충 시 아래를 따른다):**
> - 백엔드는 단일 `qtai-server`. 4 서비스 분리·K8s·Kafka·ChromaDB는 v2 분리 시 도입(보류).
> - **본인 새 역할: Bible팀 3인(이지윤·이승욱·김지민) 중 1인.** Bible 프로토타입 → Flutter → 인증 → 관리자 페이지 일괄 진행. 편집자 에이전트 보조 큐레이션·골든셋 작성도 겸임.
> - QT 본문은 성서 유니온 19:00 스크래핑(ADR-0014). 한글 본문은 대한성서공회 PDF.
> - 새번역 도입은 라이선스 명시적 확인 본인 액션.
> - 용어 "주석" → "해설" (DB·API·UI 모두 `bible_explanations`).

## 1. 내 역할

- 담당자: 이지윤
- **새 역할 (2026-05-14): Bible팀 도메인 + Flutter + 인증 + 관리자 페이지 / 편집자 에이전트 보조 큐레이션**

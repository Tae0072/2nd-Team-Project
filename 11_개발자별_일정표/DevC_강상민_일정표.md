...
연관 문서: 00_개발_일정_총괄표 / 02_ERD_문서 v1.2 / 04_API_명세서 v1.2 / 09_AI_프롬프트_운영_가이드 v1.0
...
ai-service/                         ← 전담 소유 (Python FastAPI, 포트 8085)
  ...
  │   │   ├── messages.py           (SSE 스트리밍 — ChatUseCase 연결)
...
**팀에 제공하는 공개 인터페이스**
- `POST /ai/sessions/{id}/messages` (SSE) — Flutter가 `dio_sse`로 수신
- `PATCH /ai/sessions/{id}/advance` — BFF가 라우팅
...
| 일자 | 오전 | 오후 코어 | 저녁 |
|------|------|-----------|------|
| 5/12 월 | 킥오프 참석. Python venv 구성 | `pip install -r requirements.txt`. ChromaDB K8s 배포 확인 | `rag_index.py` 실행 — collection size 확인 |
| 5/13 화 | Stand-up | Alembic 초기화. AI_SESSIONS 마이그레이션 작성 | AI_TURNS 마이그레이션 작성 |
| 5/14 수 | Stand-up | `ChatUseCase.stream_response()` — DB 연결 없이 하드코딩으로 SSE 동작 확인 | curl SSE 테스트: `curl -N http://localhost:8085/ai/sessions/1/messages` |
...
| 5/20 화 | Stand-up | `messages.py` — 세션 존재·소유권 검증 완성 | pytest 기본 테스트 작성 |
...
- [ ] `POST /ai/sessions/1/messages` → SSE 스트림 첫 토큰 수신
...
| 5/27 수 | `PATCH /ai/sessions/{id}/advance` A→B→C→D 전체 흐름 E2E 테스트 |
| 5/28 목 | `GET /ai/prompt-templates` DB 연결 완성. Bible Service 연동 (강태오·김태혁 협력) |

...
| 세션형 AI 경로 | `/ai/sessions/**`, SSE, 다중 턴 챗봇 API가 없어야 함 |
...
```powershell
npx spectral lint apis/api-v1/openapi.yaml
```
...
| `qt`/`study`/`member` 등 외부 API Controller | Repository 직접 접근 |
| `security.oauth`/공통 인증 인프라 | 도메인 비즈니스 Repository 직접 접근 |
...
| 검사 | 기준 |
| --- | --- |
| Today QT 화면 | 로그인 후 본문, 해설 진입점, 묵상 진입점, 시뮬레이터 상태 표시 |
| 로그인 경계 | 토큰이 없거나 만료되면 로그인 화면으로 이동 |
| 시뮬레이터 | `READY` 외 상태 버튼 비활성화 |
| 금지 UI | AI 자유 챗봇, AI 찬양 추천, 교회 인증 버튼 없음 |
...
| `KAKAO_CLIENT_ID` | Kakao OAuth | 환경별 분리 |
| `KAKAO_CLIENT_SECRET` | Kakao OAuth | GitHub Secrets |
...
| 다음 권장 작업 | 2번 PR `chore(ci): add qt-ai quality gates`에서 `.github/workflows/`, guard 스크립트, branch protection 기준으로 구현 |

...
- `POST /api/v1/journals/today`가 오늘 QT Journal DRAFT를 생성한다. `ai.session.completed`는 Bible Service 컨슈머가 받아 같은 `userId + qtDate` Journal에 `aiSessionId`와 AI 요약을 첨부한다.

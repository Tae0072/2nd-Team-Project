...
## Context
1차 HMS는 prompt를 코드에 하드코딩 → 변경 시 재배포 필요. AI 도메인은 prompt 튜닝이 곧 품질 — 자주 변경되는데 매번 배포는 부담. 또한 신학 가드레일이라는 도메인 특수성 때문에 검토 절차가 필요 (강상민 owner — `09_AI_프롬프트_운영_가이드.md` v1.0 작성 완료).

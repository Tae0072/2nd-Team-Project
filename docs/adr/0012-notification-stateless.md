...
## Context
AI 세션 완료 후 Bible Service가 Journal DRAFT를 자동 생성하면 사용자에게 알려야 한다. v1.0에서 별도 `NOTIFICATION_LOG` 테이블과 미수신 알림 조회 API까지 만들면 범위가 커진다. 시연에서는 사용자가 화면을 보고 있는 상황이므로 WebSocket 실시간 전달이면 충분하다.

## Decision
v1.0 알림은 BFF Aggregator의 stateless WebSocket(STOMP)으로 처리한다.

1. AI Service가 `ai.session.completed`를 발행한다.
2. Bible Service가 이벤트를 consume하여 Journal DRAFT를 생성한다.
...
별도 알림 저장 테이블은 v1.0에 만들지 않는다. 사용자가 오프라인이면 알림은 누락될 수 있지만 Journal 자체는 DB에 남아 다음 대시보드 조회에서 확인할 수 있다.

...
이 폴더는 **DevA_이지윤 (Bible팀 — Bible 도메인·해설(구 주석)·Flutter·인증·관리자 페이지 / 편집자 에이전트 보조 큐레이션)** 의 개인 작업 공간입니다.

> **2026-05-14 v2.0 재배치:** Bible팀 3인(이지윤·이승욱·김지민) 중 1인. Bible 프로토타입 → Flutter → 인증 → 관리자 페이지 일괄 진행. 편집자 에이전트 1차 검증은 AI 자동 검증으로 이관되고, 본인은 보조 큐레이션·골든셋 작성을 겸임.

## 🚫 다른 개발자(팀원)/에이전트 접근 금지

- 이 폴더(`workspaces/DevA_이지윤/`)는 **DevA_이지윤 본인만** 작성·수정·삭제할 수 있습니다.
...
- 실행 가이드: `../../11_개발자별_일정표/DevA_이지윤_실행가이드.html`

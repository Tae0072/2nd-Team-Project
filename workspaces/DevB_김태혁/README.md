...
이 폴더는 **DevB_김태혁 (시뮬레이터 단독 담당 — `com.qtai.simulator` 도메인)** 의 개인 작업 공간입니다.

> **2026-05-14 v2.0 재배치:** AI/RAG 보조 → 시뮬레이터 단독으로 이관(RAG·ChromaDB 폐기, ADR-0013). 본인은 본문 좌표 기반 장면 시뮬레이터(스프라이트 기반 검토) 단독 담당. 데이터 모델·저장소·생성 이미지 라이선스 일임.

## 🚫 다른 개발자(팀원)/에이전트 접근 금지

- 이 폴더(`workspaces/DevB_김태혁/`)는 **DevB_김태혁 본인만** 작성·수정·삭제할 수 있습니다.
...
- 실행 가이드: `../../11_개발자별_일정표/DevB_김태혁_실행가이드.html`

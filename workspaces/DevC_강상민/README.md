...
이 폴더는 **DevC_강상민 (AI 도메인 단독 주도 — `com.qtai.ai` · DeepSeek SSE · 해설 생성 · 편집자 에이전트 · 검증 메커니즘 정의)** 의 개인 작업 공간입니다.

> **2026-05-14 v2.0 재배치:** AI 주도자 강태오 → 본인으로 이관. 해설 생성 파이프라인(영어 PDF→MD→비교 데이터→자체 해설→편집자 검증)과 AI 자동 검증 메커니즘(입출력·평가 지표·합격선) 본인 단독 정의 작업. RAG/ChromaDB 폐기(ADR-0013), 출처는 사전 적재 `bible_explanations` row 참조.

## 🚫 다른 개발자(팀원)/에이전트 접근 금지

- 이 폴더(`workspaces/DevC_강상민/`)는 **DevC_강상민 본인만** 작성·수정·삭제할 수 있습니다.
...
- 실행 가이드: `../../11_개발자별_일정표/DevC_강상민_실행가이드.html`

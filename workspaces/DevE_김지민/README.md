...
이 폴더는 **DevE_김지민 (Bible팀 — Bible 도메인 · Flutter UI · 인증 · 관리자 페이지)** 의 개인 작업 공간입니다.

> **2026-05-14 v2.0 재배치 (가장 큰 역할 변동):** Flutter 단독 → Bible팀 3인 합류. Bible 프로토타입 → Flutter UI → 인증 → 관리자 페이지 일괄 진행. Flutter 빌드 책임자는 이승욱으로 이관(시연 6/17). 관리자 페이지는 W1 이후 3명 함께 진행.

## 🚫 다른 개발자(팀원)/에이전트 접근 금지

- 이 폴더(`workspaces/DevE_김지민/`)는 **DevE_김지민 본인만** 작성·수정·삭제할 수 있습니다.
...
- 실행 가이드: `../../11_개발자별_일정표/DevE_김지민_실행가이드.html`

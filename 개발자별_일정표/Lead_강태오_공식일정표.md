...
| 구분 | 고정 기준 |
| --- | --- |
| 백엔드 구조 | 단일 `qtai-server` 안에 `domain.member`, `domain.bible`, `domain.qt`, `domain.study`, `domain.note`, `domain.sharing`, `domain.praise`, `domain.ai`, `domain.admin` 등 도메인 패키지를 둔다. |
...
| 2026-05-19 | 도메인 패키지 경계 준비 | `domain.member`, `domain.bible`, `domain.qt`, `domain.study`, `domain.ai` 등 패키지 구조와 import 금지 기준 정리 | 도메인 간 Entity/Service/Repository 직접 import 금지 기준을 검증할 수 있다. |

...
> **문서 버전:** v3.1-align.2
> **작성일:** 2026-05-15 (v3.1-align.1) / 2026-05-20 (v3.1-align.2 갱신)
...
> **v3.1-align.2 변경 요약:** `03_아키텍처_정의서.md` v1.2 §13.6 강사님 직강 결정·Lead 정정 반영. `domain.audit` 단독 담당으로 명시 (관리자 행동 기록은 횡단 관심사라 Lead 직속). PR 자동화·CI/CD 인프라가 본인 전담 영역임을 §1 역할에 명시 강화.
...
| --- | --- |
| 담당자 | 강태오 |
| 주 역할 | Lead / platform / member / qt / admin / **audit (단독)** / DevOps / **PR 자동화·CI/CD 인프라 전담** / 전체 조율 |
| 구현 기준 | 단일 `qtai-server` Modular Monolith |
| 협업 기준 | 인증·Today QT·외부 API 계약은 Lead 단독 소유가 아니라 Bible·Flutter 담당자와 API 계약을 맞춰 구현한다. PR 자동화·CI/CD는 본인 전담이며 다른 사람에게 위임하지 않는다. |
| 산출물 기준 | 구현 PR, 검증 로그, 회고 메모, 문서 정합성 수정 내역, PR 자동화 워크플로 변경 이력을 남긴다. |
...
| 항목 | Lead 기준 |
| --- | --- |
| 브랜치 생성 | `dev` 최신화 후 `feature/qt-*`, `feature/member-*`, `feature/admin-*`, `feature/audit-*`, `chore/infra-*`, `chore/pr-automation-*` 형식으로 생성 |
| PR 대상 | 항상 `dev` |
